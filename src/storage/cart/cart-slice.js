...
const cartSlice = createSlice({
    ...
    reducers: {
        addProductCart: (state, action) => {
            const itemInCart = state.data.find(item => item.id === action.payload._id);
            if (itemInCart) {
                return itemInCart.quantity ++ //если товар в корзине добавляем единицу
            } else{
                state.data.push({...action.payload, quantity: 1}) // если товара нет, пушим его в корзину
            }
...
export const { addProductCart, removeProductCart, changeCartQuantity, incrementQuantity, decrementQuantity } = cartSlice.actions;
